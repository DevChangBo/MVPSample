    ...
    public void handleResponseError(Context context, Throwable t) {
        ...
        if (t instanceof CompositeException) {
            ...
            for (int i = 0; i < throwables.size(); i++) {
                if (throwables.get(i) instanceof ConnectException) {
                    stringBuffer.append("请求网络超时").append(",");
                }
                if (throwables.get(i) instanceof HttpException) {
                    HttpException httpException = (HttpException) throwables.get(i);
                    msg = convertStatusCode(httpException);
                    stringBuffer.append(msg);
                }
                if (throwables.get(i) instanceof RxCacheException) {
                    stringBuffer.append("缓存失效或不存在,请重新获取,");
                }
            }
